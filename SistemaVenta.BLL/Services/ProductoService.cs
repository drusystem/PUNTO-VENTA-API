...
namespace SistemaVenta.BLL.Services
{
    public class ProductoService:IProductoService
    {
        ...
        public async Task<ProductoDTO> Crear(ProductoDTO modelo)
        {
            try
            {
                var itemCreado = await _productoRepository.Crear(_mapper.Map<Producto>(modelo));
                if (itemCreado.IdProducto == 0)
                    throw new TaskCanceledException("No se pudo crear");

                return _mapper.Map<ProductoDTO>(itemCreado);
            }
            catch
            {
        ...
        public async Task<List<ProductoDTO>> Lista()
        {
            try
            {
                ...
                var lista = query.Include(cat => cat.IdCategoriaNavigation).ToList();   
